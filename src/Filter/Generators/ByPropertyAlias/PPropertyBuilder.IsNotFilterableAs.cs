namespace Panner.Builders //Namespace is intentional so extensions are picked up when builder is used.
{
    ...
    {
        /// <summary>Marks the property as not filterable, referenced by a specific alias.</summary>
        public static PPropertyBuilder<T> IsNotFilterableAs<T>(this PPropertyBuilder<T> pPropertyBuilder, string alias)
            where T : class
        {
